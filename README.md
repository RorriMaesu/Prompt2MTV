...
- **Music library delete** — Remove unwanted songs directly from the Music Songs Library with a single button click (with confirmation prompt).
- **ACE Step audio playback fix** — Tracks generated by ACE Step now play correctly in the timeline preview. ACE Step outputs 32-bit float WAV files which the Windows audio driver could not open; all audio is now automatically transcoded to 16-bit PCM WAV before playback.
- **Timeline scene numbers** — Scene number labels on timeline cards update instantly after drag-reorder. Previously they were frozen at the original creation order.
- **Video Generation tab sync** — Reordering clips in the Timeline Editor now also reorders the scene rows in the Video Generation tab to match.
- **Gallery scene number sync** — The "Scene N" group headers in the Gallery now reflect the current timeline order. Previously they were always grouped by original creation order regardless of how clips were rearranged.
